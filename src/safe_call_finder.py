...
input_population = widgets.Checkbox(
    value=False, description="Filter by population (population quartile)", disabled=False, style=style
)
...
def show_fms_peers(
    ...
    vis.bar_all_fm(df_data, msa, peers, fms, year=year)
    for i in fms:
...
def show_disting_peers(df_data, msa, year, n_peers, n_feat, filter_pop):
    ...
    print(f"Comparison of {code2name(msa)} and its peers for the {n_feat} most distinguishing FMs")
    pretty_prints(peers, fms)
...
def show_top_fms_peers(df_data, msa, year, n_peers, n_fms, filter_pop):
    ...
    print(f"Comparison of {code2name(msa)} and its peers for the {n_fms} most present FMs")
    pretty_prints(peers, fms)
    vis.bar_all_fm(df_data, msa, peers, fms,year)
    for i in fms:
        vis.duo_fm_viz(df_data, msa, [msa] + peers, i,year, save_fig=False, show=True)
